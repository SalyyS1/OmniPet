...
> The [wiki](https://salyys1.github.io/OmniPet/wiki.html#/roadmap) is the reference for this topic. Where this file disagrees with it, the wiki is correct.

The Gradle/persistence foundation, Admin Pet Studio slice, Phase 4 storage/economy code slice, Phase 3 dependency-neutral incubation core, Paper paid-start/recovery coordinator, player-facing hatch GUI/claim flow, and the bounded pet runtime that renders and steers active pets are implemented and verified by automated tests. The execution plan remains in progress until later slices add feeding, bonding, temporary buffs, multi-pet formations, and live gameplay evidence. Later phases must not be described as shipped until their code, tests, exact Paper matrix, migration fixtures, and live smoke evidence exist.
...
| Offline hatch administration | Shipped | `/pet admin hatch inspect\|reduce\|set\|complete\|cancel <player_name> ...` runs against persisted data with split inspect/manage permissions, caller action tokens, and non-coalesced per-player serialization. |
...
| Paper renderers and smooth movement | Shipped Phase 5 slice | One bounded main-thread coordinator with count *and* wall-clock ceilings, interpolating head renderer with explicit hard-teleport reasons, partial-spawn rollback, persisted appearance fallback, and full lifecycle cleanup. Deterministic 100-pet operation counts are recorded; live TPS/entity/network measurement remains a gate. |
| Placed-egg incubation and anticipation | Shipped | An egg placed beside a heat source incubates where it stands with a floating countdown, re-checks its requirement every second, pauses rather than accrues in an unloaded chunk, and rocks with rising intensity as hatching nears. Neighbouring eggs carry a phase offset from their block key, and an egg with nobody nearby is skipped outright. |
| Idle behaviour and per-pet temperament | Shipped | Settling when the owner stops, owner-facing yaw while idle, jittered one-shot flourishes, and a playfulness/curiosity/laziness temperament rolled deterministically from the pet's instance ID so nothing is stored and no migration is needed. A model with no rest clip keeps its idle loop; an unmapped flourish plays nothing rather than restarting the loop. |
...
| GitHub wiki | Not used | The GitHub wiki feature is enabled but empty and will stay that way. The published site at `https://salyys1.github.io/OmniPet/wiki.html` is the reference; the Markdown files in `docs/` are a convenience copy for reading in the repository, and the wiki wins where they disagree. |
