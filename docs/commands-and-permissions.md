...
| `/pet admin stats [player]` | `/petadmin stats [player]` | `omnipet.admin.reload` | Says whether that player's active pets are applying their stats, and names the reason when they are not: MythicLib absent, adapter unusable, no pet out, pet grants no stats, or a stat ID MythicLib has never registered. Read-only. Shares the reload permission rather than minting a node for one diagnostic — the person who can reload definitions is the person who needs this. |
