...
- A nameplate reads, in falling priority: the name the pet's owner gave it, the definition's own name (the Studio's **Nameplate** tile, stored as `display.name`), or the definition ID made readable. That last fallback is why every active pet carries a plate — before it, a plate only appeared for definitions whose author had found an undocumented raw-node key, so in practice none did. `nameplateStatus: false` drops the status word and uses the shorter of the two templates below. The status is only rewritten when the word itself changes, so a walking pet costs one packet per gait change rather than one per tick.
- The plate's **layout** is not in `config.yml`. It is a pair of templates in `messages.yml`, so the spacing, the ordering, the word before the level, and whether the level appears at all are yours to write:
...
- Menus and their buttons: `hub` (vault, hatch, slots, help, studio); `vault` (previous, next, hub, sort, filter, status, unlockSlot, lockedSlot, lockedSlotIdle); `hatch` (startMain, startOff, incubation, hub, refresh); `management` (favorite, lock, moveUp, moveDown, candy, breakthrough, release, refresh, back, hub); `slot` (payVault, payPoints, confirm, cancel, hub).
- `lockedSlot` is the tile the vault paints inside the pet grid, immediately after your last pet, when another active slot is for sale. `lockedSlotIdle` is that same tile when it is only informative — the cap is reached, nothing further is priced, or the gating permission is missing — and it is not clickable. Only the material is configurable; the position follows the pets, because the grid's slots are not an operator setting.
- `placedEgg` (status, reducer, instant, handIdle, refresh, hub) is the menu opened by right-clicking an egg incubating in the world. `reducer` and `instant` style the per-hand button when that hand holds a usable support item; `handIdle` styles it when it does not. There are no storage slots to configure: the item is spent straight from the player's hand rather than deposited, so the menu can never be a place an item is lost.
...
A `MODELENGINE` pet is the exception: you do not have to choose a head icon for it. Its head is never what a player sees, so saving one with the icon still untouched fills in `HEAD_CATALOG` with the definition's own ID — an untextured head. That keeps the GUI with an item to draw and leaves the HEAD fallback something to render if ModelEngine is ever unavailable. Choose a texture anyway if you want the fallback and the menu icons to look right; a texture you chose is never replaced. A `HEAD` pet still has to choose one, because there the icon *is* the pet.

Optional `display.name` is the floating name above an active pet — the Studio's **Nameplate** tile writes it. A pet with none shows its definition ID made readable, and a player who renames their pet overrides both.
