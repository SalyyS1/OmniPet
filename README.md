...
The single release artifact is `build/release/OmniPet-3.0.0-SNAPSHOT.jar` (copied from `omnipet-paper/build/libs/`). The 2026-08-03 clean JDK 21 Gradle verification checkpoint passed all 18 tasks and 160 suites/695 tests: core 63 suites/256 tests and Paper 97 suites/439 tests, with zero failures, errors, or skips. It produced a 1,726,005-byte JAR with SHA-256 `2DF199B16B0CCD1DE52DB3F30DE066BF2579A9E23212238587E50A3C51832732`: 1050 entries, 958 classes, one `paper-plugin.yml`, and zero Maven or bundled MiniMessage entries. `compileCompatibilityJava` was rerun against both `1.21` and `1.21.1` for this checkpoint because two Paper API surfaces (`org.bukkit.Sound` and `PlayerInteractEntityEvent`) entered the compile boundary. This checkpoint did not run crash-injection tests or live-server certification. Maven commands, `pom.xml`, Maven wrappers, and the old `passivepet2` artifact name are not part of the supported workflow.
