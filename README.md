...
The single release artifact is `build/release/OmniPet-3.0.0-SNAPSHOT.jar` (copied from `omnipet-paper/build/libs/`). The 2026-08-03 clean JDK 21 Gradle verification checkpoint passed all 18 tasks and 157 suites/688 tests: core 63 suites/256 tests and Paper 94 suites/432 tests, with zero failures, errors, or skips. It produced a 1,724,718-byte JAR with SHA-256 `6087903B33300D605E97DB30B6DB715D494C2AA0889FE6528D2C431C61237C5B`: 1049 entries, 957 classes, one `paper-plugin.yml`, and zero Maven or bundled MiniMessage entries. `compileCompatibilityJava` was rerun against both `1.21` and `1.21.1` for this checkpoint because two Paper API surfaces (`org.bukkit.Sound` and `PlayerInteractEntityEvent`) entered the compile boundary. This checkpoint did not run crash-injection tests or live-server certification. Maven commands, `pom.xml`, Maven wrappers, and the old `passivepet2` artifact name are not part of the supported workflow.
