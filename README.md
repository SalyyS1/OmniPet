...
- Paper registers `/pet` with `/pets` as its alias. `/pet` opens a player hub (vault, hatch, active slots, help, and for staff the Studio); `/pet vault [page]`, `/pet <page>`, `/pet hatch [main|off|claim|refresh|use-main|use-off]`, and the `/pet admin browse` Studio branch remain directly reachable. `/pet help [page]` and permission-filtered tab-complete come from one declarative command tree.
...
- `/pet admin browse` and `/pets admin browse` open the D/C/B/A/S Studio tier browser for `omnipet.admin.managepet` staff. The Studio supports paginated definition lists, a reflection-safe MythicLib stat picker with manual fallback, create/edit drafts, typed rarity/progression/skill/release fields, archive mode, safe inventory events, chat input expiry/cancel, optimistic conflict checks, and atomic Save/reload generation swaps. Every Studio chat field prompts with its format, an example, and the `cancel` hint; clicking a stat shows one explained button per supported modifier; the head-icon field auto-detects a pasted base64 payload, an `http(s)` URL, or a bare 64-character texture hash.
- `/pet [page]` and `/pets [page]` open the UUID/revision-safe player vault. The vault cycles five sort orders (favorites-first, level, rarity, name, recent) and four filters (all, favorites, active, stored) on a click, over the already-loaded snapshot with no extra read; every comparator is total and ties on pet UUID, so no pet can appear on two pages or on none. An empty vault, a filter matching nothing, and the last page each say what they are instead of looking identical. One plugin-owned `PerPlayerTaskQueue` is injected into the vault and slot-purchase controllers: accepted work runs FIFO per player across both controllers, while different players may run concurrently. Only pending reads with the matching namespaced key (`vault:view` or `slot:view`) coalesce; reconciliation, mutations, and purchases never coalesce. Repository work stays async; Bukkit inventory, permission, and provider work stays on the main thread.
...
- Every state change now produces a sound: activating a pet, buying a slot, a rejected purchase, and a blocked request are audibly distinct. Cues play to the acting player only, are rate-limited per player so a held click cannot become an audible nuisance, sit beside the existing chat message rather than replacing it, and switch off entirely with `gui.feedback.enabled: false`.
...
- Right-clicking a vault pet — or your own rendered pet in the world — opens management: favorite, lock, reorder, EXP candy, breakthrough, and release. The world click reuses the shipped activation index through a lookup that rejects stale renderer generations, filters the off-hand event so one click opens one screen, and leaves a non-OmniPet entity uncancelled. Every rendered inventory binds viewer, owner, pet UUID, session, generation, and expected revision; drags touching the top inventory are cancelled and a late async result cannot replace a newer view.
...
The following remain phased work: live provider/server certification, crash-injection and process-kill certification, riding, entity click interaction and passive event triggers, Active Party, rename control, MMOItems cultivation identities, a live MythicMobs skill picker in the Studio, an optional head-catalog provider, admin target mode for another player's pets, and a separately versioned addon/live API. See [the roadmap](docs/roadmap.md) for ownership and release gates.
...
The single release artifact is `build/release/OmniPet-3.0.0-SNAPSHOT.jar` (copied from `omnipet-paper/build/libs/`). The 2026-08-03 clean JDK 21 Gradle verification checkpoint passed all 18 tasks and 158 suites/691 tests: core 63 suites/256 tests and Paper 95 suites/435 tests, with zero failures, errors, or skips. It produced a 1,726,005-byte JAR with SHA-256 `2DF199B16B0CCD1DE52DB3F30DE066BF2579A9E23212238587E50A3C51832732`: 1050 entries, 958 classes, one `paper-plugin.yml`, and zero Maven or bundled MiniMessage entries. `compileCompatibilityJava` was rerun against both `1.21` and `1.21.1` for this checkpoint because two Paper API surfaces (`org.bukkit.Sound` and `PlayerInteractEntityEvent`) entered the compile boundary. This checkpoint did not run crash-injection tests or live-server certification. Maven commands, `pom.xml`, Maven wrappers, and the old `passivepet2` artifact name are not part of the supported workflow.
