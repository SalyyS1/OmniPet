...
The single release artifact is `build/release/OmniPet-3.0.0-SNAPSHOT.jar` (copied from `omnipet-paper/build/libs/`). The 2026-08-03 clean JDK 21 Gradle verification checkpoint passed all 18 tasks and 147 suites/605 tests: core 63 suites/256 tests and Paper 84 suites/349 tests, with zero failures, errors, or skips. It produced a 1,674,717-byte JAR with SHA-256 `485AC4C6AA25042B3CBD4D2805C4D76B317F37AAACFE7326639D89D86B4299A9`: 1015 entries, 925 classes, one `paper-plugin.yml`, and zero Maven or bundled MiniMessage entries. This checkpoint did not rerun compatibility probes, crash-injection tests, or live-server certification. Maven commands, `pom.xml`, Maven wrappers, and the old `passivepet2` artifact name are not part of the supported workflow.
