...
The single release artifact is `build/release/OmniPet-3.0.0-SNAPSHOT.jar` (copied from `omnipet-paper/build/libs/`). The 2026-08-03 clean JDK 21 Gradle verification checkpoint passed all 18 tasks and 161 suites/699 tests: core 63 suites/256 tests and Paper 98 suites/443 tests, with zero failures, errors, or skips. It produced a 1,727,430-byte JAR with SHA-256 `2D945A3ED5D3B64B2D8C90D17D70FD8A4C67EF235BA4BC44AD0C46BA04A373F7`: 1051 entries, 959 classes, one `paper-plugin.yml`, and zero Maven or bundled MiniMessage entries. `compileCompatibilityJava` was rerun against both `1.21` and `1.21.1` for this checkpoint because two Paper API surfaces (`org.bukkit.Sound` and `PlayerInteractEntityEvent`) entered the compile boundary. This checkpoint did not run crash-injection tests or live-server certification. Maven commands, `pom.xml`, Maven wrappers, and the old `passivepet2` artifact name are not part of the supported workflow.
